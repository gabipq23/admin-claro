...
export class OffersService {
  async allOffersFiltered({
    ...
    const res = await apiPurchase.get(`/telecom/claro/offers`, {
      params: {
        pagina: page,
        date_from: date_from,
        date_to: date_to,
  ...
  async inputOffers(file: File, description: string): Promise<unknown> {
    ...
    const response = await apiPurchase.post(`/telecom/claro/offers`, formData, {
      headers: {
  ...
  async downloadOfferFile(id: number, fileName: string): Promise<void> {
    const response = await apiPurchase.get(
      `/telecom/claro/offers/${id}/download`,
    );
  ...
  async updateOffers(id: number, data: UpdateMonthOfferData): Promise<unknown> {
    const response = await apiPurchase.put(`/telecom/claro/offers/${id}`, data);
    return response.data;
  }

  async removeOffers(id: number) {
    await apiPurchase.delete(`/telecom/claro/offers/${id}`);
  }
}
